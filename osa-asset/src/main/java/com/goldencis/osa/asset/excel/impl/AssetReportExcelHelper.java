 ...
 **/
@Component
public class AssetReportExcelHelper implements IImport<HSSFWorkbook>, IExport<IExport.Builder<HSSFWorkbook, ResourceEntity>> {

    private final Logger logger = LoggerFactory.getLogger(AssetReportExcelHelper.class);
    ...
    public String sheetName() {
        return AssetSheet.ASSETREPORT.tag();
    }
    ...
    //导出
    @Override
    public void export(Builder<HSSFWorkbook, ResourceEntity> builder) throws Exception {
        ...
        List<ResourceEntity> list = builder.getList();
        if (CollectionUtils.isEmpty(list)) {
            throw new Exception("列表为空");
        }
        List<IHeader> headerList = headerProvider.provide(ResourceEntity.class);
    ...
    private void drawLine(ResourceEntity item, HSSFRow row, HSSFCellStyle contentStyle) throws Exception {
        Class<? extends ResourceEntity> clazz = item.getClass();
