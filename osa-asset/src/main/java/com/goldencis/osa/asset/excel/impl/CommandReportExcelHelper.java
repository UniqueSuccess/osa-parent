 ...
 **/
@Component
public class CommandReportExcelHelper implements IImport<HSSFWorkbook>, IExport<IExport.Builder<HSSFWorkbook, CommandItem>> {

    private final Logger logger = LoggerFactory.getLogger(CommandReportExcelHelper.class);
    ...
    public String sheetName() {
        return AssetSheet.COMMANDREPORT.tag();
    }
    ...
    //导出
    @Override
    public void export(Builder<HSSFWorkbook, CommandItem> builder) throws Exception {
        ...
        List<CommandItem> list = builder.getList();
        if (CollectionUtils.isEmpty(list)) {
            throw new Exception("列表为空");
        }
        List<IHeader> headerList = headerProvider.provide(CommandItem.class);
    ...
    private void drawLine(CommandItem item, HSSFRow row, HSSFCellStyle contentStyle) throws Exception {
        Class<? extends CommandItem> clazz = item.getClass();
