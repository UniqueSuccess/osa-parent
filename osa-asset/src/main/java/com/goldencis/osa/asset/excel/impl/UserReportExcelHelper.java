 ...
 **/
@Component
public class UserReportExcelHelper implements IImport<HSSFWorkbook>, IExport<IExport.Builder<HSSFWorkbook, UserReportItem>> {
    ...
    //导出
    @Override
    public void export(Builder<HSSFWorkbook, UserReportItem> builder) throws Exception {
        ...
        List<UserReportItem> list = builder.getList();
        if (CollectionUtils.isEmpty(list)) {
            throw new Exception("列表为空");
        }
        List<IHeader> headerList = headerProvider.provide(UserReportItem.class);
    ...
    private void drawLine(UserReportItem item, HSSFRow row, HSSFCellStyle contentStyle) throws Exception {
        Class<? extends UserReportItem> clazz = item.getClass();
