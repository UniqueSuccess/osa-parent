...
var app = new Vue({
    ...
    data: {
        ...
        toolbarConfig: [{
                ...
                title: "导出",
                action: function() {
                    var exportUrl = "/report/userReportExport?"
                    initExport("resourceTable","user",exportUrl);
                }
            },
            {
                type: 'searchbox',
        ...
        tableConfig: {
            ...
            ajax: {
                //其它ajax参数同jquery
                url: baseUrl + '/report/getUserReportList',
                //改变从服务器返回的数据给table
...
function getEcharsData(getTime,search) {
    ...
    var numTotal = 0,
        legendData = [],
        seriesData = [];
    $.get(baseUrl + '/report/getUserReportChart', { dateTime: getTime, searchStr:search}, function(msg) {
        if (msg.resultCode == 0) {
